...
describe('SettingsModalManager', () => {
  ...
  describe('openSettingsModal', () => {
    it('should load and display settings', async () => {
      ...
      expect(mockElements.waitForOptionsMilliseconds.value).toBe('100');
      expect(mockElements.autoFillProgressDialogMode.value).toBe('withCancel');
      expect(mockElements.logLevel.value).toBe('1');
      expect(mockElements.enableTabRecording.checked).toBe(false);
      expect(mockElements.enableAudioRecording.checked).toBe(false);
      expect(mockElements.recordingBitrate.value).toBe('2500');
      expect(mockElements.recordingRetentionDays.value).toBe('10');
      expect(mockElements.gradientStartColor.value).toBe('#4F46E5');
      expect(mockElements.gradientStartColorText.value).toBe('#4F46E5');
      expect(mockElements.gradientEndColor.value).toBe('#7C3AED');
      expect(mockElements.gradientEndColorText.value).toBe('#7C3AED');
      expect(mockElements.gradientAngle.value).toBe('135');
      expect(mockElements.gradientAngleDisplay.textContent).toBe('135°');
